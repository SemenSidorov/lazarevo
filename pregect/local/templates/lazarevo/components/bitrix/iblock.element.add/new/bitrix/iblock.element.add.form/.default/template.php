						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								for ($i = 0; $i<$inputNum; $i++)
								{
									...
									?>
									<span><?if (intval($propertyID) > 0){?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?}else{?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?}?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])) echo "*";?>:</span>
									<input
									type="text"
									name="PROPERTY[<?=$propertyID?>][<?=$i?>]"
									id="PROPERTY_INPUT_<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] : $propertyID?>"
									placeholder="<?if (intval($propertyID) > 0){?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?}else{?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?}?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])) echo "*";?>"
