...
<div class="top-text">
	...
	Регистрация в личном кабинете предназначена только для заказа товара на нашем сайте.
</div>
<div class="mail">
...
<form method="post" id="regform" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data">
<?
if($arResult["BACKURL"] <> ''):
?>
	<input type="hidden" name="backurl" value="<?=$arResult["BACKURL"]?>" />
...
<?
		...
		{
		case "PASSWORD":
			?><input placeholder="<?=$arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "*" : ""?><?=GetMessage("REGISTER_FIELD_".$FIELD)?>" size="30" type="password" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" class="bx-auth-input" />
		...
		case "CONFIRM_PASSWORD":
			?><input placeholder="<?=$arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "*" : ""?><?=GetMessage("REGISTER_FIELD_".$FIELD)?>"
		...
		default:
			...
			?><input placeholder="<?=$arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "*" : ""?><?=GetMessage("REGISTER_FIELD_".$FIELD)?>" size="30" type="text" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" /><?
...
<input type="checkbox" class="checkbox" id="agree"><label id="labelAgree" for="agree">Я даю согласие на обработку персональных данных и получение информации от Лазаревская Лавка</label>
<div class="button">
	<input type="submit" name="register_submit_button" value="<?=GetMessage("AUTH_REGISTER")?>" />
	<p><?echo $arResult["GROUP_POLICY"]["PASSWORD_REQUIREMENTS"];?></p>
	<p><span class="starrequired">*</span><?=GetMessage("AUTH_REQ")?></p>
	<p><a href="/personal/auch/">Авторизация</a></p>
</div>
